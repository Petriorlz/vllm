...
from vllm.multimodal.inputs import (MultiModalDataDict, MultiModalFieldConfig,
                                    MultiModalKwargs)
from vllm.multimodal.parse import (AudioProcessorItems, AudioEmbeddingItems, MultiModalDataItems,
...
from .utils import (AutoWeightsLoader, WeightsMapper, init_vllm_registered_model,
...
class KimiAudioEmbeddingInputs(TypedDict):
...
class KimiAudioProcessor:
    ...
    def __call__(self, text: str, audio: list, return_tensors) -> torch.Tensor:
        print('KimiAudioProcessor', text, audio, return_tensors)
        inputs = self.tokenizer(text)
        print(f"{inputs = }")
        audio_tensor = torch.tensor(numpy.array(audio))
        inputs.update({
...
class KimiAudioProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(
        ...
        **kwargs: object,
        ...
        # FIXME: no real processor
        # return self.ctx.get_hf_processor(Qwen2AudioProcessor, **kwargs)
        return KimiAudioProcessor(self.get_tokenizer())
...
class KimiAudioDummyInputsBuilder(
    ...
    def get_dummy_text(self, mm_counts: Mapping[str, int]) -> str:
        ...
        # FIXME: no hf processor
        # hf_processor = self.info.get_hf_processor()
        # audio_token = hf_processor.audio_token
        audio_token = "<|im_kimia_text_blank|>"
    ...
    def get_dummy_mm_data(
        ...
        # FIXME: Kimi has no feature extractor
...
class KimiAudioMultiModalProcessor(
    ...
    def _call_hf_processor(
        ...
        # print('call_hf_processor', mm_data, mm_kwargs)
    ...
    def _get_prompt_updates(
        ...
        audio_token = getattr(processor, "audio_token", "<|im_kimia_text_blank|>")
        ...
        def get_replacement_qwen2_audio(item_idx: int):
            ...
            audios = mm_items.get_items("audio", (AudioProcessorItems, AudioEmbeddingItems))
            if isinstance(audios, AudioEmbeddingItems):
                num_features = audios.get_feature_size(item_idx)
...
class KimiAudioForConditionalGeneration(nn.Module, SupportsMultiModal,
    ...
    def get_multimodal_embeddings(self,
        ...
        if 'audio_embeds' in kwargs:
            if isinstance(kwargs['audio_embeds'], list):
                # return kwargs['audio_embeds']
                return [embed.squeeze(0) for embed in kwargs['audio_embeds']]
            # FIXME: batch 假设 embeds 等长
            # print(f"{kwargs['audio_embeds'] = }")
            return kwargs['audio_embeds'].squeeze(1)
        if 'input_features' not in kwargs:
            return []
        # print(f"{kwargs['input_features'].shape = }")
        batch_size, _, _ = kwargs['input_features'].shape
        dummy_embeddings = torch.zeros((batch_size, 1500, self.config.hidden_size), dtype=kwargs['input_features'].dtype, device=kwargs['input_features'].device)
    ...
    def get_input_embeddings(
        ...
        if multimodal_embeddings is not None:
            print(f"{multimodal_embeddings[0].shape = }")
        else:
            # 我们假设这代表生成阶段
            # 增加 audio channel 的 pad 以符合训练情况
            audio_pad_ids = torch.full_like(input_ids, 151666)
            audio_embeds = self.language_model.get_input_embeddings(audio_pad_ids)
        ...
        if multimodal_embeddings is not None \
            ...
            inputs_embeds = merge_multimodal_embeddings(
                input_ids, inputs_embeds, multimodal_embeddings,151666) 
                # <|im_kimia_text_blank|>
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        loader = AutoWeightsLoader(self, skip_prefixes=["language_model.model.mimo_layers", "language_model.model.mimo_norm", "mimo_output"])
